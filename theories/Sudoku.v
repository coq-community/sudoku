...
Proof.
  ...
  intros x y s; rewrite Nat.add_comm, jump_add.
...
Proof with auto with arith.
  ...
  rewrite div_mult_comp, (mod_small (div y w) h), (div_is_0 (div y w) h),  Nat.add_0_r...
  match goal with
...
Proof with auto with arith.
  ...
  - rewrite H1; clear H1.
    ...
    + repeat rewrite Nat.add_assoc.
      repeat (apply Nat.add_le_mono || apply Nat.mul_le_mono_l || apply Nat.mul_le_mono_r);
        auto with arith.
...
Definition check_P: forall (P: nat -> Prop) (P_dec: forall i, {P i} + {~ P i}) n,
  ...
  left; intros i Hi; case (Lt.le_lt_or_eq_stt i n2); try intros Hi1; subst; auto with arith.
...
Definition lit_test (v1 v2: lit) :=
  match v1 with
    L p1 v1 => match v2 with
      L p2 v2 => match pos_test p1 p2 with
        eq =>  test v1 v2
      | r  => r
      end
    end
...
Proof with auto.
  intros l1 l2 l3; case l1; case l2; case l3; simpl...
...
Proof.
  ...
  apply fold_insert1 with (f := (fun p => (L (shift p (h * div i h) (w * mod i h)) z))); auto.
...
Theorem anti_literals_in:
  forall p z1 z2, In z2 ref_list -> z2 <> z1 -> In (L p z2) (anti_literals (L p z1)).
Proof.
...
Definition valid_lit l s :=
  match l with L p z => ~ In (get p s) ref_list /\ valid_pos p /\ In z ref_list end.
...
Theorem clauses_update_valid:
  forall p z c cs s, valid_lit (L p z) s -> ordered cs ->
                ordered_clause c ->
...
Proof with auto with arith.
  ...
  case (Lt.le_lt_or_eq_stt (pos2n p1) (pos2n p)); try intros V2.
...
Theorem anti_literals_sat:
  forall p z s, sudoku s -> valid_pos p -> get p s = z -> ~ clause_sat (anti_literals (L p z)) s.
Proof with auto with arith.
...
Proof with auto with arith.
  ...
  intros p1 Hp1.
  case (Nat.le_gt_cases (pos2n p) (pos2n p1)); intros Hp2.
  case Lt.le_lt_or_eq_stt with (1 := Hp2); clear Hp2; intros Hp2; subst.
