...
assert (F1: div y w < h).
  {
    apply div_lt; rewrite  Nat.mul_comm; auto.
  }
...
- generalize size; intros m.
    ...
    * intros n1 _ l.
      ...
      ++rewrite nth_app_l.
        ...
        **case (Nat.le_gt_cases (length l) w); intros H2.
          *** rewrite length_take_small; auto.
  ...
  + intros n1 Rec h1; case h1; simpl.
    ...
    * intros h2 HH l.
      ...
      ++rewrite nth_app_r.
        **rewrite length_take_small, jump_too_far; auto.
          *** rewrite take_and_jump_nil.
              repeat rewrite nth_default; auto with arith.
              apply Nat.le_trans with (1 := H3); auto with arith.
          *** apply Nat.le_trans with (1 := H3); auto with arith.
        **rewrite length_take_small; auto with arith.
        ++rewrite nth_app_r.
          *** rewrite length_take; auto with arith.
              repeat rewrite <- Nat.add_assoc; 
              rewrite (Nat.add_comm w), Nat.add_sub; auto with arith.
              rewrite jump_nth, jump_add, <- jump_nth; auto with arith.
          *** rewrite length_take; auto with arith.
- unfold size; nia.
Qed.
    ...
    * intros b s1 Rec s p cs H0 H1 H2 H3 H4.
      ...
      --assert (F1: pos2n (next p) < length s).
        ++case (Nat.le_gt_cases (length s) (pos2n (next p))); auto; intros H5.
          rewrite (jump_too_far _ (pos2n (next p)) s) in F0;
          ...
          **apply Rec; auto.
            ...
            --- rewrite prestrict_update; auto with arith.
                ...
                +++ apply clauses_update_valid; auto.
                    ...
                    *** apply anti_literals_ordered.
                    *** intros; apply anti_literals_in; auto.
                +++ unfold get; rewrite <- H1; auto with arith.
          **apply Rec; auto.
            --- apply valid_pos_next; auto.
                 rewrite <- H0; auto.
            --- intros n c Hn (p1, z1) Hl; simpl.
                ...
                +++ rewrite prestrict_get_default; auto.
                    apply out_not_in_refl.
                +++ rewrite prestrict_get; auto with arith.
      ...
      --apply Nat.add_le_mono.
        ++apply Nat.mul_le_mono_r; auto.
          ...
          case h; auto with arith; intros; simpl minus;
              repeat rewrite Nat.sub_0_r; auto with arith.
        ++match goal with
          |- mod ?X ?Y <= ?T =>
...
- assert (U2: 0 < w).
  + generalize V1; unfold size; case w; simpl; auto with arith.
    rewrite Nat.mul_0_r; intros tmp; contradict tmp; auto with arith.
...
{
  apply (V2 n c); auto with datatypes.
}
...
- intros s1 Hr1.
  ...
  + intros Hs.
    ...
    * apply clauses_update_sat_rev; auto.
      ...
      --destruct (V6 s1 Hrr) as (Hrr1, _).
        ...
        intros n1 c1 Hn1c1.
        apply (Hrr2 n1 c1); auto with datatypes.
  ...
  + intros (Hs, Hp).
    ...
    * injection H1; intros HL Hn.
      ...
      rewrite <-HL; auto.
    * apply clauses_update_sat with (4 := Hs) (5 := H1); auto.
      apply anti_literals_ordered; auto with datatypes.
Qed.
...
- intros p1 Hv1.
  case (pos_dec p p1); intros Heq; try (subst p1).
  + rewrite (update_get p z s); try apply valid_pos2n; auto.
    ...
    rewrite (update_get p z s) in Hc2; try apply valid_pos2n; auto.
    subst z1.
  ...
  + rewrite update_diff_get; auto.
    ...
    case (pos_dec p p2); intros Heq1; try (subst p2).
    * red in Hc2; rewrite update_get in Hc2; try apply valid_pos2n; auto.
      subst z2.
  ...
  + intros Hs1.
    ...
    * apply clauses_update_sat_rev; auto.
      ...
      --intros [p1 z1] HIl Hl.
        ...
        case (H3 p); auto.
        exists (L p1 (get p1 s1)); split; auto.
        red; auto.
      --destruct (V6 s1 Hrr) as (Hrr1, _).
        destruct (Hrr1 Hs1) as (Hrr2, _); auto.
    * intros p1 Hp1 Hr.
      case (iff_impl_l _ _ (sudoku_def s1) Hs1); intros H1 [H2 H3].
      apply H3; auto.
  + intros (Hs, Hp).
...
- case (V6 s2 Hrs2); auto; intros _ tmp; apply tmp; clear tmp.
  ...
  intros n2 c2; simpl; intros [HH | HH].
  + injection HH; intros; subst; clear HH.
    case (Hs2 n2 c); auto with datatypes.
    intros l (Hl1, Hl2); exists l; auto with datatypes.
  + apply (Hs2 n2); auto with datatypes.
Qed.
    ...
    invariant (gen_init_clauses_aux s1 p cs) s).
{
  ...
  intros s cs p H H0 H1 H2 H3; rewrite prestrict_all in H2; auto.
  - case (Nat.le_gt_cases (length s) (pos2n p)); auto.
    intros H4; absurd (length (jump (pos2n p) s) = length (@nil nat)).
    + generalize (length_jump _ (pos2n p) s).
      rewrite <- H3; simpl.
              ...
              --- intros p1 Hp1.
                  ...
                  +++ case Lt.le_lt_or_eq_stt with (1 := Hp2); clear Hp2; 
                      ...
                      *** rewrite prestrict_get; auto.
                        ...
                        ----rewrite <- Hp2; rewrite next_pos; auto.
    ...
    * repeat rewrite (fun l x => (jump_nth nat l (pos2n x))).
      ...
      --split; auto.
        ++apply div_lt; auto.
          rewrite <- Hr1; auto.
        ++apply mod_lt; auto.
          generalize H4; rewrite Hr1; case size; auto with arith.
          intros HH; contradict HH; auto with arith.
      --assert (F1 : div n size < size). 
        ...
        {
          apply mod_lt; auto.
          generalize H4; rewrite Hr1; case size; simpl; auto with arith.
          intros tmp; contradict tmp; auto with arith.
        }
...
- intros a n Rec s [|[n1 [|l c1]] cs].
  ...
  + intros Hl Hi.
    ...
    * intros s1 cs1 Hl1 Hi1.
      ...
      --apply Nat.le_trans with (1 := Hl1); auto.
...
Lemma find_one_aux_correct cs s cs1 :
match find_one_aux cs s cs1 with
| None => find_all_aux cs s cs1 = nil
| Some s1 => In s1 (find_all_aux cs s cs1)
end.
Proof.
revert s cs1; elim cs; auto; clear cs.
- intros s [|(n, [|l c1]) cs1]; simpl; auto.
- intros c cs IH s [|(n, [|l c1]) cs1]; auto.
  + simpl; auto.
  + simpl; auto.
  + unfold find_one_aux; fold find_one_aux.
    unfold find_all_aux; fold find_all_aux.
    revert s cs1; elim (l :: c1); clear l c1.
    * simpl; auto.
    * intros [p z] c1 IH1 s cs1.
      unfold try_one; fold try_one.
      unfold try_all; fold try_all.
      generalize (IH 
         (update p z s) (clauses_update (L p z) (anti_literals (L p z)) cs1)).
      case find_one_aux.
      --intros l1 H1.
        apply merge_incl_l; auto with datatypes.
      --intros H1; rewrite H1.
        generalize (IH1 s cs1); case try_one.
        ++intros l2 H2.
          apply merge_incl_r; auto with datatypes.
          apply lexico_exact.
          apply test_exact.
        ++intros H2; rewrite H2; auto.
Qed.

Theorem find_one_correct_aux s :
  length s = size * size ->
  match find_one s with
  | None => find_all s = nil
  | Some s1 => In s1 (find_all s)
  end.
Proof.
intros Hs; unfold find_one, find_all.
generalize (check_init_state_correct _ Hs); case check_init_state; auto.
intros _.
...
Proof.
intros Hs.
generalize (find_one_correct_aux s Hs); case find_one.
- intros s1 H1.
  destruct (iff_impl_r _ _ (find_all_correct s s1 Hs) H1) as [H2 H3]; auto.
- intros H1 s1 Hs1 Hs2.
  assert (H2 : In s1 (find_all s)) by (apply find_all_correct; auto).
  rewrite H1 in H2; auto with datatypes.
Qed.

(* Proof of just one_correct vs all_correct                                   *)

Lemma find_just_one_aux_correct cs s cs1 :
match find_just_one_aux cs s cs1 with
| jNone => find_all_aux cs s cs1 = nil
| jOne s1 =>
    find_all_aux cs s cs1 = s1 :: nil
| jMore s1 s2 =>
    In s1 (find_all_aux cs s cs1) /\
    In s2 (find_all_aux cs s cs1) /\
    s1 <> s2
end.
Proof.
revert s cs1; elim cs; auto; clear cs.
...
- intros c cs IH s [|(n, [|l c1]) cs1]; auto.
  + simpl; auto.
  + simpl; auto.
  + unfold find_just_one_aux; fold find_just_one_aux.
    unfold find_all_aux; fold find_all_aux.
    revert s cs1; elim (l :: c1); clear l c1.
    * simpl; auto.
    * intros [p z] c1 IH1 s cs1.
      unfold try_just_one; fold try_just_one.
      unfold try_all; fold try_all.
      generalize (IH 
         (update p z s) (clauses_update (L p z) (anti_literals (L p z)) cs1)).
      case find_just_one_aux.
      --intros H1; rewrite H1.
        generalize (IH1 s cs1); case try_just_one.
        ++intros H2; rewrite H2; auto.
        ++intros l H2; rewrite H2; auto.
        ++intros l1 l2 [H2 H3]; split; auto.
      --intros l1 H1; rewrite H1.
        generalize (IH1 s cs1); case try_just_one.
        ++intros H2; rewrite H2; auto.
        ++intros l2 H2; rewrite H2; auto.
          generalize (list_nat_eq_correct l1 l2); case list_nat_eq; auto.
          **intros Hl1l2; subst l2; simpl.
            elim l1; simpl; auto.
            intros n1 l2 IH2.
            generalize (test_anti_sym n1 n1); case test; intros; 
              try discriminate.
            generalize IH2; case lexico; intros; try discriminate; auto.
          ** intros Dl1l2; split; [|split]; auto.
             --- apply merge_incl_l; auto with datatypes.
             --- apply merge_incl_r; auto with datatypes.
                 apply lexico_exact.
                 apply test_exact.
        ++intros l2 l3 [H2 [H3 H4]].
          split; [|split]; auto.
          **apply merge_incl_r; auto with datatypes.
            apply lexico_exact.
            apply test_exact.
          **apply merge_incl_r; auto with datatypes.
            apply lexico_exact.
            apply test_exact.
      --intros l1 l2 [H1 [H2 H3]]; split; [|split]; auto.
        ++apply merge_incl_l; auto with datatypes.
        ++apply merge_incl_l; auto with datatypes.
Qed.

Theorem find_just_one_correct_aux s :
  length s = size * size ->
    match find_just_one s with 
     jNone =>  find_all s = nil
   | jOne s1 => find_all s =  (cons s1 nil)
   | jMore s1 s2 =>  In s1 (find_all s) /\
                     In s2 (find_all s) /\
                     s1 <> s2
   end.
Proof.
intros Hs; unfold find_just_one, find_all.
generalize (check_init_state_correct _ Hs); case check_init_state; auto.
intros _.
...
Proof.
intros Hs.
generalize (find_just_one_correct_aux s Hs); case find_just_one.
- intros H1 s1 Hs1 Hs2.
  assert (H2 : In s1 (find_all s)) by (apply find_all_correct; auto).
  rewrite H1 in H2; auto with datatypes.
- intros s1 H1.
  assert (H2 : In s1 (find_all s)) by (rewrite H1; auto with datatypes).
  destruct (iff_impl_r _ _ (find_all_correct s s1 Hs) H2) as [H3 H4].
  split; auto; split; auto.
  intros s2 H5 H6.
  assert (H7 : In s2 (find_all s)) by (apply find_all_correct; auto).
  rewrite H1 in H7; inversion H7 as  [|[]]; auto.
- intros s1 s2 [H1 [H2 H3]].
  destruct (iff_impl_r _ _ (find_all_correct s s1 Hs) H1) as [H4 H5].
  destruct (iff_impl_r _ _ (find_all_correct s s2 Hs) H2) as [H6 H7].
  auto.
Qed.
